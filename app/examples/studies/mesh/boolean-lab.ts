...
export default function (container: HTMLElement) {
  ...
  return modelingLabView(
    container,
    "Boolean Lab · experimental",
    () => {
      const { a, b } = booleanOperands(params);
      try {
        return {
          geometry: experimentalBoolean(a, b, params.operation),
          guides: [a, b],
          message: `${params.operation} · A ${params.operation === "Subtract" ? "−" : params.operation === "Union" ? "∪" : "∩"} B\nBlue = A · terracotta = B\nOwn BSP splitter; study-local and size-limited.\nInspect closure before trusting the result.`,
        };
      } catch (error) {
        a.dispose();
        b.dispose();

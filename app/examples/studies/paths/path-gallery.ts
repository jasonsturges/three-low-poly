...
import { arcPath } from "three-low-poly";
import { curvePath } from "three-low-poly";
import { helixPath } from "three-low-poly";
import { linePath } from "three-low-poly";
import { type PathPoint } from "three-low-poly";
import { joinPaths, transformPath } from "three-low-poly";
import { spiralPath } from "three-low-poly";
import { circleProfile } from "three-low-poly";
import { sweep, transportFrames } from "three-low-poly";
import { createScene } from "../../../framework/createScene";

export const meta: ExampleMeta = {
  title: "Path Gallery",
  description:
    "STUDY — one profile, every path. A sweep has two halves, a PROFILE and a PATH, and neither knows the " +
    "other exists: change the profile and a masonry arch becomes wrought-iron tubing; change the path and " +
    "the same tubing becomes a ring, a corkscrew, a scroll, an archway. That independence is the whole " +
    "primitive, and it is easiest to believe when you watch the profile hold still. Every generator in " +
    "@lib/paths appears here — line, arc, ring, C, helix, spiral, sampled curve, and a composed archway " +
    "built by joining three of them.",
};

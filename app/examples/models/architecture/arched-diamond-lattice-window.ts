...
import { ArchedDiamondLatticeWindow } from "three-low-poly";
import { clearDefaultLights } from "../../../framework/clearDefaultLights";
import { createScene } from "../../../framework/createScene";

export const meta = {
  title: "Arched Diamond Lattice Window",
  description: "Diamond cames analytically clipped to a Tudor arch — real geometry, correct shadow, extruded frame ring.",
};
...
export default function (container: HTMLElement) {
  ...
  const gui = new GUI();
  gui.title("Arched Diamond Lattice");

  const openingFolder = gui.addFolder("Opening");
  openingFolder.add(params, "width", 2, 8).name("Width").step(0.1).onChange(rebuild);
  openingFolder.add(params, "rectHeight", 1.5, 8).name("Rect Height").step(0.1).onChange(rebuild);
  openingFolder.add(params, "archHeight", 0.5, 5).name("Arch Height").step(0.1).onChange(rebuild);
  openingFolder.open();
  ...
  latticeFolder.add(params, "cellsY", 1, 24).name("Cells Y (full)").step(1).onChange(rebuild);

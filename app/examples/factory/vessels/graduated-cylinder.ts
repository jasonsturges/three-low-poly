...
import { LiquidFillGeometry, GraduatedCylinderGeometry } from "three-low-poly";
import { createScene } from "../../../framework/createScene";
import { frameObject } from "../../../framework/frameObject";
import { gradientBackdrop } from "../../../framework/gradientBackdrop";

export const meta = { title: "Graduated Cylinder" };
...
export default function (container: HTMLElement) {
  ...
  let shellGeometry = new GraduatedCylinderGeometry({
  ...
  const rebuildShell = () => {
    ...
    shellGeometry = new GraduatedCylinderGeometry({
      radius: params.radius,
      height: params.height,
      radialSegments: params.radialSegments,
      rim: params.rim,
  ...
  const gui = new GUI();
  gui.title("Graduated Cylinder");
  gui.add(params, "radius", 0.15, 0.8, 0.01).name("Radius").onChange(rebuildShell);

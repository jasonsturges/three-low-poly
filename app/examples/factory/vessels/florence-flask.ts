...
import { LiquidFillGeometry, FlorenceFlaskGeometry } from "three-low-poly";
...
export default function (container: HTMLElement) {
  ...
  let shellGeometry = new FlorenceFlaskGeometry({
  ...
  const rebuildShell = () => {
    ...
    shellGeometry = new FlorenceFlaskGeometry({
      bodyRadius: params.bodyRadius,
      neckRadius: params.neckRadius,
      neckHeight: params.neckHeight,
      radialSegments: params.radialSegments,
  ...
  const gui = new GUI();
  gui.title("Florence Flask");
  gui.add(params, "bodyRadius", 0.3, 2, 0.01).name("Body Radius").onChange(rebuildShell);
  gui.add(params, "neckRadius", 0.05, 0.6, 0.01).name("Neck Radius").onChange(rebuildShell);
  gui.add(params, "neckHeight", 0.2, 4, 0.01).name("Neck Height").onChange(rebuildShell);

...
import { FlorenceFlaskGeometry, createLiquidFill, centerObject } from "three-low-poly";
...
export default function (container: HTMLElement) {
  ...
  const build = () => {
    clear();
    const geometry = new FlorenceFlaskGeometry({
  ...
  gui.add(params, "neckRadius", 0.05, 0.6, 0.01).name("Neck Radius").onChange(build);
  gui.add(params, "neckHeight", 0.2, 4, 0.01).name("Neck Height").onChange(build);
